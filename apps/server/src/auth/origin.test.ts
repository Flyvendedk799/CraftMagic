...
const PUBLIC_ORIGIN = 'https://imaginecraft.example';
...
describe('originGuard', () => {
	it('refuses a mutation from a foreign origin', async () => {
		for (const method of ['POST', 'PUT', 'PATCH', 'DELETE']) {
			expect(
				await run(method, { origin: 'https://evil.example', host: 'imaginecraft.example' }),
			).toBe(403);
		}
	});

	it('allows the configured public origin', async () => {
		expect(await run('POST', { origin: PUBLIC_ORIGIN, host: 'imaginecraft.example' })).toBeNull();
	});
	...
	it('allows a request with no Origin at all', async () => {
		...
		expect(await run('POST', { host: 'imaginecraft.example' })).toBeNull();
	});
	...
	it('refuses the literal string "null", which a sandboxed frame sends', async () => {
		...
		expect(await run('POST', { origin: 'null', host: 'imaginecraft.example' })).toBe(403);
	});
	...
	it('does not guard reads', async () => {
		...
		expect(await run('GET', { origin: 'https://evil.example', host: 'imaginecraft.example' })).toBeNull();
		expect(await run('HEAD', { origin: 'https://evil.example', host: 'imaginecraft.example' })).toBeNull();
	});

	it('refuses a malformed Origin', async () => {
		expect(await run('POST', { origin: 'not a url', host: 'imaginecraft.example' })).toBe(403);
	});
	...
	it('accepts loopback on any port outside production, and not inside it', async () => {
		...
		expect(await run('POST', { origin: 'http://localhost:5183', host: 'imaginecraft.example' }, true)).toBe(403);
	});
});
